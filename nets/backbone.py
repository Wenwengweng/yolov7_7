...
class My_MCB(nn.Module):
    def __init__(self, c1, c2, c3, n=4, e=1, ids=[0]):
        super(My_MCB, self).__init__()
        c_ = int(c2 * e)
        ...
        self.cv3 = Conv(2 * c_, c_, 1, 1)
        self.cv4 = Conv(c_, c3, 1, 1)
    ...
    def forward(self, x):
        ...
        a = self.cv3(torch.cat((x_1, x_2), dim=1))
        # print(a.shape)
        for i in range(self.n):
            x_2 = self.cv2(x_2)
            a = self.cv3(torch.cat((x_2, a), dim=1))
            # print(a.shape)
